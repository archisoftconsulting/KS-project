...
//			obj.add(new BasicDBObject("firstName", firstName));	
			obj.add(new BasicDBObject("firstName", Pattern.compile(firstName, Pattern.CASE_INSENSITIVE)));
		}
		if(lastName != null && lastName.length() > 0) {
//			obj.add(new BasicDBObject("lastName", lastName));
			obj.add(new BasicDBObject("lastName", java.util.regex.Pattern.compile(lastName, Pattern.CASE_INSENSITIVE)));
		}		
...
//		JsonNode jsonData = mapper.convertValue(result, JsonNode.class);
	...
	public Result listStudentsByPage() {
		
		String firstIndex = request().queryString().get("firstIndex")[0];
		String rowPerPage = request().queryString().get("rowPerPage")[0];
		
		int first = Integer.parseInt(firstIndex);
		int last = first + Integer.parseInt(rowPerPage);
		
		Set<Student> result = StudentStore.getInstance().getAllStudents();
		ObjectMapper mapper = new ObjectMapper();
		
		BasicDBObject searchQuery = new BasicDBObject();
		DBCursor cursor = getUsersTable().find(searchQuery).skip(first).limit(last);		
